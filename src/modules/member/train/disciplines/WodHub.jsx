...
// (formats, intent, length, benchmarks, timer). The three new disciplines
...
// Wrapper that injects the user's 5K PB for running
function SimpleDisciplineWithRunning(props) {
  const { state } = useApp()
  // Look for a 5K PB in profile (fiveKSeconds), or fall back to null
  const fiveKSec = state.profile?.fiveKSeconds || null
  return <SimpleDiscipline {...props} context={{ fiveKSec }} />
}
